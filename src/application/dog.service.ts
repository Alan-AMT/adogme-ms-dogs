...
export class DogService {
  ...
  async createDog(
    ...
    try {
      ...
      const imageInstances = this.createImagesDomainInstances(
        dogId,
        createDogDto.amountImagesToCreate ?? 0,
      );
      const [dogTags, dogMl, uploadUrls] = await Promise.all([
        ...
        this.imagesPort.generateUploadLinks(
          dogId,
          imageInstances.map((image) => image.id),
        ),
      ]);
      const dogToCreate = Dog.createDog({
        ...
        vaccinations: this.createVaccinationsDomainInstances(
          dogId,
          createDogDto.vaccinations,
        ),
  ...
  async findAll(): Promise<Dog[]> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch all dogs: ${error.message}`,
        error.stack,
      );
  ...
  async findAllCatalog(query: GetDogsCatalogDto): Promise<{
    ...
    try {
      ...
      const { data, total } = await this.repository.findAllCatalog(
        filters,
        page,
        limit,
      );
      ...
      this.logger.error(
        `Failed to fetch dogs catalog: ${error.message}`,
        error.stack,
      );
  ...
  async findAllByShelterId(
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch dogs for shelter ${shelterId}: ${error.message}`,
        error.stack,
      );
  ...
  async findDogById(dogId: string): Promise<Dog> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch dog ${dogId}: ${error.message}`,
        error.stack,
      );
  ...
  async updateDog(
    ...
    this.logger.log(
      `Attempting to update dog ${dogId} for owner ${userOwnerId}`,
    );
    try {
      const dog = await this.repository.findDogById(dogId);
      if (!dog) {
      ...
      if (dog.userOwnerId !== userOwnerId) {
        this.logger.warn(
          `Owner ${userOwnerId} attempted to update dog ${dogId} without permission`,
        );
      ...
      const [dogTags, dogMl, newImages] = await Promise.all([
        ...
        this.mlDogPort.updateMlDog(
          updateDogDto,
          adoptionFee ?? 0,
          dogId,
          dog.images.length +
            (amountImagesToCreate ?? 0) -
            (imagesToDelete?.length || 0),
        ),
        this.getUpdatedAndSortedDogImages(
          dogId,
          dog.images,
          imagesToDelete,
          amountImagesToCreate ?? 0,
      ...
      const updatedDog = Dog.createDog({
        ...dog,
        ...dogData,
        personality: dogTags,
        vaccinations: updateDogDto.vaccinations
          ? this.createVaccinationsDomainInstances(
              dogId,
              updateDogDto.vaccinations,
            )
          : [],
      ...
      const [uploadUrls, _] = await Promise.all([
        this.imagesPort.generateUploadLinks(
          ...
          newImages
            .filter(
              (image) =>
                !dog.images.map((dogImage) => dogImage.id).includes(image.id),
            )
            .map((image) => image.id),
        ),
      ...
      if (
        ...
        throw error;
      this.logger.error(
        `Failed to update dog ${dogId}: ${error.message}`,
        error.stack,
      );
  ...
  async updateDogStatus(
    ...
    this.logger.log(
      `Attempting to update dog ${dogId} status to ${status} for owner ${userOwnerId}`,
    );
    try {
      const dog = await this.repository.findDogById(dogId);
      if (!dog) {
      ...
      if (dog.userOwnerId !== userOwnerId) {
        this.logger.warn(
          `Owner ${userOwnerId} attempted to update dog ${dogId} status without permission`,
        );
        throw new ForbiddenException('No puedes editar este perro');
      }
      ...
      if (
        ...
        throw error;
      this.logger.error(
        `Failed to update dog ${dogId} status: ${error.message}`,
        error.stack,
      );
  ...
  async createAndGetPersonalityTags(
    dogId: string,
    dtoTags: PersonalityDto[],
  ): Promise<PersonalityTag[]> {
    const existingTags = await this.repository.findPersonalityTagsByLabel(
      dtoTags.map((tag) => tag.label),
    );
    const notExistingTags = dtoTags.filter(
      (tag) =>
        !existingTags.some((existingTag) => existingTag.label === tag.label),
    );
  ...
  createVaccinationsDomainInstances(
    dogId: string,
  ...
  createImagesDomainInstances(dogId: string, amountImages: number): DogImage[] {
    ...
    const BUCKET_NAME_PUBLIC = 'adogme-dogs-compressed';
    let images: DogImage[] = [];
    for (let i = 0; i < amountImages; i++) {
      const imageId = uuidv4();
      images.push(
        DogImage.createImage({
          id: imageId,
          dogId: dogId,
  ...
  async updateImageStatus(imageId: string, status: ImageStatus): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to update image status for ${imageId}: ${error.message}`,
        error.stack,
      );
  ...
  async deleteDog(dogId: string, userOwnerId: string): Promise<void> {
    try {
      ...
      if (dog.userOwnerId !== userOwnerId) {
        this.logger.warn(
          `Owner ${userOwnerId} attempted to delete dog ${dogId} without permission`,
        );
      ...
      if (
        ...
        throw error;
      this.logger.error(
        `Failed to delete dog ${dogId}: ${error.message}`,
        error.stack,
      );
  ...
  async getUpdatedAndSortedDogImages(
    dogId: string,
    dogImages: DogImage[],
    imagesToDelete: string[] = [],
    amountImagesToCreate: number = 0,
    ...
    const newImages = this.createImagesDomainInstances(
      dogId,
      amountImagesToCreate,
    );

    // 3. Filter remaining existing images
    const remainingImages = dogImages.filter(
    ...
    if (updatedMainImageId) {
      const found = remainingImages.find(
        (img) => img.id === updatedMainImageId,
      );
      if (!found) {
    ...
    if (!newMainImage) {
      newMainImage = newImages[0] ?? remainingImages[0] ?? null;
    }
  ...
  async getPortraitDogs(): Promise<DogFindAllCatalog[]> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch portrait dogs: ${error.message}`,
        error.stack,
      );
  ...
  async findDogsByIds(dogIds: string[]): Promise<DogFindAllCatalog[]> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to fetch dogs by ids: ${error.message}`,
        error.stack,
      );
      throw new InternalServerErrorException('Failed to fetch dogs');
    }
  ...
  async updateDogsShelterData(
    shelterId: string,
    dto: UpdateDogsShelterDataDto,
  ): Promise<void> {
    this.logger.log(`Updating shelter data for shelter ${shelterId}`);
    try {
      await this.repository.updateDogsShelterData(
        shelterId,
        dto.shelterName,
        dto.shelterLogo,
      );
      this.logger.log(
        `Successfully updated shelter data for shelter ${shelterId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to update shelter data for shelter ${shelterId}: ${error.message}`,
        error.stack,
      );
      throw new InternalServerErrorException(
        'Failed to update shelter data for dogs',
      );
    }
  }
}
