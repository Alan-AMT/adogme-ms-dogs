...
export class GoogleOidcGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const expectedServiceAccountEmail = this.configService.get<string>('OIDC_SERVICE_ACCOUNT_EMAIL');

    if (!expectedAudience || !expectedServiceAccountEmail) {
      this.logger.error('OIDC_AUDIENCE or OIDC_SERVICE_ACCOUNT_EMAIL is not configured');
      throw new UnauthorizedException('Internal server error: OIDC configuration missing');
    }
    ...
    try {
      ...
      if (payload.email !== expectedServiceAccountEmail) {
         this.logger.warn(`Unauthorized service account access attempt from: ${payload.email}`);
