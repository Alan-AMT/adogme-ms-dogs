...
export class CloudStorageAdapter implements ImagesPort {
  ...
  async generateUploadLinks(
    ...
    try {
      ...
      const BUCKET_NAME_UPLOADS = 'adogme-dogs';

      if (!BUCKET_NAME_UPLOADS) {
        throw new Error(
          'BUCKET_NAME_UPLOADS is not defined in the environment config',
        );
      }
  ...
  async deleteDogImages(dogId: string): Promise<void> {
    ...
    try {
      //   const BUCKET_NAME_PUBLIC =
      //     this.configService.get<string>('BUCKET_NAME_PUBLIC');
      const BUCKET_NAME_PUBLIC = 'adogme-dogs-compressed';

      if (!BUCKET_NAME_PUBLIC) {
        throw new Error(
          'BUCKET_NAME_PUBLIC is not defined in the environment config',
        );
      }
