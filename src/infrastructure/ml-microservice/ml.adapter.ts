...
export class MlDogAdapter implements MlDogPort {
  ...
  async createMlDog(
    ...
    try {
      const parsedDog = this.parseDogToMlPayload(
        dog,
        adoptionFee,
        dogId,
        dog.amountImagesToCreate ?? 0,
      );
      ...
      const response = await fetch(
        `https://adogme-ml-1087097859781.us-central1.run.app/ml-ms/predict/process-dog`,
        {
          method: 'POST',
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `Machine Learning service error! status: ${response.status}`,
        );
      }
  ...
  async updateMlDog(
    ...
    try {
      ...
      const response = await fetch(
        // `${process.env.ML_SERVICE_URL}/predict/process-dog`,
        `https://adogme-ml-1087097859781.us-central1.run.app/ml-ms/predict/process-dog`,
        {
          method: 'PUT',
          headers: {
  ...
  async deleteMlDog(dogId: string): Promise<void> {
    try {
      ...
      const response = await fetch(
        // `${process.env.ML_SERVICE_URL}/dogs/${dogId}`,
        `https://adogme-ml-1087097859781.us-central1.run.app/ml-ms/dogs/${dogId}`,
        {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
  ...
  async refreshTokenClient(): Promise<void> {
    try {
      ...
      const client = await auth.getIdTokenClient(
        ...
        'https://adogme-ml-1087097859781.us-central1.run.app/ml-ms',
      );
